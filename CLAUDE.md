...
- 2026-06-04 to 2026-06-13 — Slices 3-7 (see Build Slices table); Parking UPI/Cash split, sales-based Show Summary + Profit/Net Profit cards, Cash Breakdown form, per-item Wastage table, PDF enhancements, 12hr time picker, ShowCard badge/edit fix
- 2026-06-15 — HistoryPage.tsx rewritten as calendar-first design (month calendar + selected-day panel + Recent strip, replacing flat paginated list); follow-ups: renamed `today`→`todayIST` for clarity, widened recent-days window to 30 distinct dates so filter-tab counts cover multi-day theatre gaps while the Recent strip still shows only the most recent 7; flagged known caveat that `totalSales` is derived from slip UPI/Cash payment-split columns, not item-level sale×price, so a day can show ₹0 if payment splits were never entered
- 2026-06-15 (cont.) — Removed Parking Summary section from Day Close (dropped unused `parkingGap`/`parkingMissing` fields, kept scooter/auto/car counts + parkingExpected/parkingReported for ShowSummaryTable + PDF); added delete-show button (Trash2 icon) with `show_number` re-sequenced by `start_time` on both insert and delete; ShowPage OB carry-forward now gated independently per slip (mc/pc/cd) and finds the previous show via `start_time` order instead of `show_number`; CoolDrinksSlip reordered to Drinks items → Drinks Payment → Live Counter divider → Live items → Live Counter Payment → MiscDrinksRow → Cool Drinks Total
