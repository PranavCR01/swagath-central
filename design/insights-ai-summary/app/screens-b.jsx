...
function DayCloseScreen({ theatre, close, setClose, onBack, onSummary }) {
  ...
  return (
    <>
      ...
      <div style={{ flexShrink: 0, padding: '14px 18px', borderTop: '1px solid var(--card-border)', background: 'linear-gradient(180deg, var(--surface), var(--bg))' }}>
        ...
        {onSummary && (
          <button onClick={onSummary} className="t-card-tap" style={{ width: '100%', marginTop: 12, height: 52, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, background: 'color-mix(in srgb, var(--accent) 13%, transparent)', border: '1px solid color-mix(in srgb, var(--accent) 36%, transparent)', borderRadius: 'var(--r-btn)', color: 'var(--accent)', fontFamily: 'inherit', fontSize: 15, fontWeight: 650, cursor: 'pointer' }}>
            <Icon name="sparkle" size={18} color="var(--accent)" />Generate AI Summary
