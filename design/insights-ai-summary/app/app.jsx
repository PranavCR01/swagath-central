        ...
        <div className="app-root" style={{ width: dev.w, height: dev.h, ...appVars }}>
          <DeviceFrame width={dev.w}>
            ...
            <div key={nav.screen + (nav.showId || '')} className="screen-anim" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
              ...
              {nav.screen === 'close' && theatre && <DayCloseScreen theatre={theatre} close={data.dayClose[nav.theatreId]} setClose={setClose} onBack={() => go('day', { theatreId: nav.theatreId })} onSummary={() => go('aisummary', { theatreId: nav.theatreId })} />}
              {nav.screen === 'aisummary' && theatre && <AiSummaryScreen theatre={theatre} onBack={() => go('close', { theatreId: nav.theatreId })} />}
            </div>
